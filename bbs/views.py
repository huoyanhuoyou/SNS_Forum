...
def article(request, article_id, page):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_comment.save()
            return redirect('bbs:article', article_id=article_id, page = 1)
...
def search(request):
    ...
    if condition == 'content':
        articles = Article.objects.filter(text__contains=keyword).order_by('-date_added')
    elif condition == 'title':
        articles = Article.objects.filter(title__contains=keyword).order_by('-date_added')
    elif condition == 'plate':
        articles = Article.objects.filter(plate__text__contains=keyword).order_by('-date_added')
    elif condition == 'date':
        articles = Article.objects.filter(date_added__contains=keyword).order_by('-date_added')
    elif condition == 'owner':
        articles = Article.objects.filter(owner__username__contains=keyword).order_by('-date_added')
    else:
        articles = []
    ...
    if articles:
        start = (page - 1) * page_size
        end = start + min(page_size, length - start)
        data = articles[start:end]
        articles = data
